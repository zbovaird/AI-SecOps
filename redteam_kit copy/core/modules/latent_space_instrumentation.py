...
class ModelInstrumentation:
    ...
    def _register_forward_hook(self, name: str, module: nn.Module):
        """Register forward hook for a specific module"""
    ...
    def _register_attention_hook(self, name: str, module: nn.Module):
        """Register hook specifically for attention mechanisms"""
        def attention_hook_fn(module, input, output):
            ...
            if isinstance(output, tuple):
                ...
                if len(output) > 0:
                    self.attention_outputs[name]['output'] = output[0].detach().clone()
                if len(output) > 1:
                    self.attention_outputs[name]['weights'] = output[1].detach().clone()
            else:
    ...
    def export_activations(self, output_path: str, format: str = 'numpy'):
        ...
        if format == 'numpy':
            np.savez_compressed(output_path, **{
                name: [a.cpu().numpy() for a in acts]
