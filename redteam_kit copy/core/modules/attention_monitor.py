...
class AttentionMonitor:
    ...
    def _register_attention_hook(self, name: str, module: nn.Module):
        """Register hook for a specific attention module"""
        def forward_hook(module, input, output):
            # Try to extract Q, K, V
            self._extract_qkv(name, module, input)
            
            # Extract attention weights and outputs
            if isinstance(output, tuple):
                if len(output) > 0:
                    self.attention_outputs[name].append(output[0].detach().clone())
                if len(output) > 1:
                    self.attention_weights[name].append(output[1].detach().clone())
            else:
                self.attention_outputs[name].append(output.detach().clone())
